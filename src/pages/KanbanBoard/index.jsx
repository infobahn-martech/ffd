...
export default function KanbanBoard() {
  ...
  const hideAllTooltips = useCallback(() => {
    // Hide tooltip containers - react-tooltip v5 uses elements with id starting with "react-tooltip-"
    const tooltipContainers = document.querySelectorAll('[id^="react-tooltip-"]');
    ...
    // Also check for elements with react-tooltip class
    const tooltipByClass = document.querySelectorAll('[class*="react-tooltip"]');
    tooltipByClass.forEach(tooltip => {
  ...
  // Hide all tooltips when CardForm modal opens (with continuous monitoring)
  useEffect(() => {
    if (selectedCard) {
      // Hide immediately
      hideAllTooltips();
      
      // Set up interval to continuously hide tooltips while modal is open
  ...
  }, [selectedCard, hideAllTooltips]);

  // Hide all tooltips when context menu opens
  useEffect(() => {
    if (contextMenu) {
      hideAllTooltips();
      const interval = setInterval(() => {
  ...
  }, [contextMenu, hideAllTooltips]);

  // Hide all tooltips when accordion menu opens
  useEffect(() => {
    if (accordionMenu) {
      hideAllTooltips();
      const interval = setInterval(() => {
