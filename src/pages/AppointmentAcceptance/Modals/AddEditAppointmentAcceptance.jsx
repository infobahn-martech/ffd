...
if (!Quill.__tableBetterRegistered) {
  ...
  // quill/formats/image.js). The float/margin wrap style the layout dropdown
  // below sets on the <img> would silently vanish the next time saved content
  // gets reloaded (this modal's edit-mode load runs HTML through
  // quill.clipboard.convert(), which rebuilds the Delta purely from formats()).
  // Extending it to also carry `style` is what quill's own clipboard matcher
  // (matchBlot in modules/clipboard.js) expects for round-tripping.
...
// Quill image inserts in the CG Pass / Vessel Registration template modals.
...
// QuillEditor — mounts a real Quill v2 instance onto a bare div ref, bypassing
// react-quill's controlled-component reconciliation. Accepts value (HTML string)
// and calls onChange(html) on every text-change. Mirrors the pattern used in
// the CG Pass / Vessel Registration template modals for the same Quill v2 +
// quill-table-better combo — this modal is left-to-right only (no RTL toggle).
function QuillEditor({ value, onChange, placeholder }) {
  ...
  useEffect(() => {
    ...
    // Prevents insertTable from silently aborting inside a Bootstrap modal
...
export function ReportTemplatesModal({
  ...
  return (
    <CustomModal
      className="report-template-modal"
