...
import { FiX, FiPlus, FiChevronDown, FiChevronUp, FiTrash2, FiFilter, FiUsers, FiInfo, FiCalendar, FiDownload, FiMaximize2, FiShare2 } from 'react-icons/fi';
...
function BusinessRuleFormModal({ show, rule: ruleProp, businessRuleId, boardName, onClose, onSave, isSaving, isCopyMode }) {
  ...
  const handleOpenCreateDetails = (id) => {
    ...
    const action = createActions.find((a) => a.id === id);
    const isRelational = RELATIONAL_CREATE_ACTION_LABELS.includes(action?.label?.trim().toLowerCase());
  ...
  const handleSaveCreateDetails = (destination) => {
    ...
    if (action && RELATIONAL_CREATE_ACTION_LABELS.includes(action.label?.trim().toLowerCase())) {
      ...
      // Plain "Create card" — once a destination is picked, move straight into the
      // title/description/card-fields panel instead of leaving the action half-configured.
      setShowCreateCardDetailsModal(true);
    }
  };
